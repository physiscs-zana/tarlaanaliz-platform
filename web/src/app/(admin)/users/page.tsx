    ...
    <section className="space-y-4">
      ...
      {users.length === 0 && !error ? (
        <div className="rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">Kayitli kullanici bulunmamaktadir.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-slate-100 bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-2 text-left font-medium text-slate-600">Il</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {users.map((u) => (
                <tr key={u.user_id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2.5">{u.province}</td>
