    ...
    <section className="space-y-4">
      ...
      {payments.length === 0 && !error ? (
        <div className="rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">Bekleyen odeme bulunmamaktadir.</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
          <table className="w-full text-sm">
            <thead className="border-b border-slate-100 bg-slate-50">
              <tr>
                <th className="px-4 py-2 text-left font-medium text-slate-600">Odeme ID</th>
                <th className="px-4 py-2 text-left font-medium text-slate-600">Tutar</th>
                <th className="px-4 py-2 text-left font-medium text-slate-600">Yontem</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {payments.map((p) => (
                <tr key={p.payment_id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-500">{p.created_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
